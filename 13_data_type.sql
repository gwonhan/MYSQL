...
select convert(avg(menu_price), signed integer ) from tbm_menu;
...
select 5 > '6May'; -- 문자에서 첫 번째로 나온 숫자는 정수로 전환된다.
select 5 > '6May'; -- 숫자가 뒤에 나오면 문자로 인식되어 0으로 변환한다.
