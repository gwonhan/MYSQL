...
-- cte : 쿼리내부(1개의 쿼리문)에서 with ~as 절을 사용해서 "임시" 로 정의되는 테이블

-- select 쿼리문을 저장한 객체로 가상 테이블이라고 불린다.
-- 실질적인 데이터를 물리적으로 저장하지 않고, 쿼리만 저장했지만
-- 테이블을 사용하는 것고 동일하게 사용할 수 있다.

-- view 는 데이터를 쉽게 읽어 이해할 수 있도록 돕는 동시에,
...
-- 2. 뷰에 포함되지 않은 컬럼 중 데이터 테이블 컬럼이 not null 제약조건이 지정된 경우
-- 3. 산술표현식이 정의된 경우
-- 4. join 을 이용해 여러 테이블을 연결한 경우
-- 5. distinct 를 포함한 경우
-- 6. 구룹함수(avg, sum 등등)나 group by 절을 포함한 경우
