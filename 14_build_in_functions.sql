...
-- concat (문자열1, 문자열2, ...)
-- concat _ws(구분자ㅡ 문자열 1, 문자열2, ...)

select concat('호랑이', '기린','토끼' );
select concat('-', '2024','03', '04' );

-- elt : 해당 위치의 문자열 반환
-- field : 찾을 문자열 위치 반환
-- find in_set : 찾을 문자열의 위치 반환
-- instr : 기준 문자열에서 부분 문자열의 시작 위치 반환
-- locate : intstr 과 동일하고 순서만 반대
...
-- format (숫자, 소수점 자리수)
-- 1000 단위마다 콤마(,) 표시를 해주며 소주점 아래 자릿수 까지 반올림 표현한다.    

select format(1.13213213231,3);
...
-- lpad(문자열, 길이, 채울 문자열), rpad(문자열ㄹ,길이, 채울 문자열)
-- 문자열을 길이만큼 오니쪽으로 늘린 후에 빈 곳을 문자열로 채운다.
-- 001313- 1******
...
select ceiling(1234.56), floor(1324.56), round(1234.56);

-- mod(숫자1, 숫자2) or 숫자2 or 숫자1 mod 숫자2
-- 숫자1을 숫자2로 나눈 나머지 추출 
...
-- truncate(숫자 정수);
-- 소수점을 기준으로 정수 위치까지 구하고 나머지 버림 - 는 왼쪽으로 줄어듬
select truncate(12345.1235, 4);

-- 날짜 및 시간 관련 함수
-- addate(날짜, 차이) : 날짜를 기준으로 차이를 더함 
...
select subtime('2024-03-04 17:42:00', '0:30:0');
...
-- 연 월 일 시 분초 밀리초 전부 가능
select year(curtime()),month(curdate()), microsecond(curdate());


-- datediff(날짜 1, 날짜2)
-- timediff(날짜 1 or 시간 1, 날씨2 또는 시간 2)
-- datediff : 날짜 1 - 날짜 2의 일수를 반환
-- timediff : 시간 1 - 시간 2의 결과
select datediff('2024-03-07', now()),timediff('19:05:11', '13:00:10');


-- dayofweek(날짜) : 요일 반환 (일요일이 1)
-- monthname : 해당 달의 이름 반환
-- dayofyear : 해당 년도에서 몇 일이 흘렀는지
